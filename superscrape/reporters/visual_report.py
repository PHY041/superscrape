...
def _generate_recommendations(
    ...
    if top_type == "white_bg" and type_dist.get("white_bg", 0) > 60:
        ...
        recs.append(
            f"Lifestyle images lead ({type_dist['lifestyle']}%). Match this trend but differentiate with unique scenes."
        )

    # Text overlays
    text_pct = _pct(text_count, total)
    if text_pct > 50:
        recs.append(f"{text_pct}% of competitors use text overlays/callouts. Add key feature callouts to your images.")
    elif text_pct < 20:
        recs.append(f"Only {text_pct}% use text overlays. Adding feature callouts could be a differentiator.")
    ...
    if person_pct > 40:
        ...
        recs.append(f"Only {person_pct}% show people. Adding a person using the product could increase relatability.")

    # Angles
    if angle_dist.get("front", 0) > 70:
        recs.append("Front-facing angles dominate. Try a 45-degree angle for dimensionality.")
